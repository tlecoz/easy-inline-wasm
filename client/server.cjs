...
app.post('/compile', upload.none(), async (req, res) => {
    ...
    const name = sanitize(req.query.name || 'last_module');
  ...
  try {
    ...
    // 5. Renvoie le binaire wasm au client ET sauvegarde dans public/
    ...
    const wasmDir = path.join(__dirname, 'public', 'wasms');
